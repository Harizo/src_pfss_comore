(function ()
{
    ...
    function notification($cookieStore,apiFactory,$interval,loginService)
    {
        ...
        if (id_user > 0) 
        {
            ...
            apiFactory.getAPIgeneraliserREST("count_contrat_agep/index","menu","count_by_sp","id_sous_p",Number(1)).then(function(result) 
            {
                var x = result.data.response;
                vs.content = x.length;
                vs.color = '#F44336' ;
                ...
                if (id_user) 
                {
                    ...
                    $interval(function(){apiFactory.getAPIgeneraliserREST("count_contrat_agep/index","menu","count_by_sp","id_sous_p",Number(1)).then(function(result) 
                    {
                        var resultat = result.data.response;
