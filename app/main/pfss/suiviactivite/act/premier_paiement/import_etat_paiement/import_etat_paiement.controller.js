(function ()
{
    'use strict';
    angular
        .module('app.pfss.suiviactivite.suivi_act.premier_paiement.import_etat_paiement')
        ...
        .controller('Import_etat_paiementController', Import_etat_paiementController);

    /** @ngInject */
    function Import_etat_paiementController(apiFactory, $state, $mdDialog, $http, $scope,$location,$cookieStore,apiUrl,apiUrlExcel,apiUrlExcelimport) {
		var vm = this;
	   vm.dtOptions =
      {
      ...
      //initialisation variable
		...
		if(vm.url=='/suivi-activite/act/premier-paiement/enregistrer-etat-de-paiement') {
		...
		if(vm.url=='/suivi-activite/act/deuxieme-paiement/enregistrer-etat-de-paiement') {
     ...
     vm.filtre_region = function()
      ...
      vm.filtre_village = function()
      {
		...
		vm.filtrer = function()	{
			...
			apiFactory.getAPIgeneraliserREST("requete_export/index","fiche_paiement",1,"village_id",vm.filtre.village_id).then(function(result) { 
     ...
     vm.filtre_region_detail = function()
      ...
      vm.filtre_village_detail = function()
      {
		...
		vm.filtrer_detail = function()	{
			vm.affiche_load = true ;
			apiFactory.getAPIgeneraliserREST("requete_export/index","liste_etat_paiement",1,
																	"village_id",vm.filtredetail.village_id,
																	"id_sous_projet",vm.filtredetail.id_sous_projet,
																	"agep_id",vm.filtredetail.agep_id,
		...
		vm.modifier_etat_presence= function() {
			vm.all_etatpresence.forEach(function(obj) {
				if(vm.filtre.fichepresence_id==obj.id) {
					vm.filtre.microprojet_id=obj.microprojet_id;
					vm.filtre.agex_id=obj.agex_id;
				}
			});			
		}
		...
		vm.Importer_etat_paiement= function () {
			vm.affiche_load = true ;
			apiFactory.getAPIgeneraliserREST("requete_import/index",
                                                ...
                                                "fichepresence_id",vm.filtre.fichepresence_id,
                                                "etat_paiement",1,
                                                "importation",1,
		...
		vm.getTotalPaye = function(){
			...
			if(vm.all_fiche_presence.length >0) {
				for(var i=0;i < vm.all_fiche_presence.length;i++) {
					var product = vm.all_fiche_presence[i];
					total += parseFloat(product.montanttotalpaye) ;
				}
			}	
			return total;
		}		
